...
public class ComponentParameters : IComponentParameters
{
    ...
    public void Add(string parameterName, object value) => _parameters[parameterName] = value;

    public T Get<T>(string parameterName) => _parameters.TryGetValue(parameterName, out var value)
            ? (T)value
